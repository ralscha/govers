...
import (
	...
	"github.com/ralscha/govers/mongodb"
	"github.com/testcontainers/testcontainers-go"
	tcmongodb "github.com/testcontainers/testcontainers-go/modules/mongodb"
)

const testSalaryProperty = "salary"
...
func setupTestContainer(t *testing.T) (*mongodb.Repository, func()) {
	...
	container, err := tcmongodb.Run(ctx,
		...
		testcontainers.WithEnv(map[string]string{
			"MONGO_INITDB_DATABASE": "govers_test",
		}),
	)
	if err != nil {
	...
	repo, err := mongodb.NewWithConnString(ctx, connString, "govers_test")
	if err != nil {
		_ = container.Terminate(ctx)
...
func TestUpdateCommit(t *testing.T) {
	...
	for _, change := range commit.Changes {
		if vc, ok := change.(core.ValueChange); ok && vc.PropertyName == testSalaryProperty {
			salaryChange = &vc
			break
		}
	}
	...
	if salaryChange.PropertyName != testSalaryProperty {
...
func TestQueryByDateRange(t *testing.T) {
	...
	beforeCommit := time.Now().Add(-time.Second)
	if _, err := g.Commit(ctx, "test-user", emp); err != nil {
		t.Fatalf("Failed to commit: %v", err)
	}
	afterCommit := time.Now().Add(time.Second)

	query := core.ByInstanceIDQuery("Employee", 1).
		From(beforeCommit).
		To(afterCommit).
		Build()
	...
	query = core.ByInstanceIDQuery("Employee", 1).
		From(afterCommit.Add(time.Hour)).
		Build()
...
func TestQueryByChangedProperty(t *testing.T) {
	repo, cleanup := setupTestContainer(t)
	defer cleanup()
	...
	query := core.ByInstanceIDQuery("Employee", 1).
		WithChangedProperty(testSalaryProperty).
		Build()
	...
	if len(snapshots) < 1 {
		t.Errorf("Expected at least 1 snapshot with salary change, got %d", len(snapshots))
	}
	...
	if !found {
		t.Error("No snapshot found with 'salary' in changed properties")
	}
}
...
func TestCustomCollectionNames(t *testing.T) {
	...
	repo, err := mongodb.NewWithConnString(ctx, connString, "govers_test",
		mongodb.WithSnapshotsCollectionName("custom_snapshots"),
		mongodb.WithHeadIDCollectionName("custom_head"),
	)
	if err != nil {
		t.Fatalf("Failed to create repository: %v", err)
	}
